...
public class BillRepository extends RepositoryBase<Bill> {
	...
	public Bill getById(int billId) throws SQLException {
		Cursor cursor = db.query(true, BillTable.TABLE_NAME,
	...
	private Bill inflateBill(Cursor mCursor) {
		...
		bill.customerName = mCursor.getString(mCursor.getColumnIndexOrThrow(BillTable.KEY_CUSTOMERNAME));
		return bill;
	}
	...
	public Bill[] selectAll() {
		...
		cursor.close();
		return result.toArray(new Bill[result.size()]);
	}
	...
	protected ContentValues createContentValues(Bill bill) {
		...
		values.put(BillTable.KEY_CUSTOMERNAME, bill.customerName);
